...
export const SideNavigation: React.FC<SideNavigationProps> = ({ children }) => {
  ...
  return (
    <div className="h-full w-full flex items-center justify-center">
      <div className="w-[1200px] max-w-screen my-16 grid grid-cols-[1fr_clamp(1rem,100vw,680px)_1fr] xl:grid-cols-[1fr_680px_1fr] grid-rows-auto auto-cols-fr xl:gap-4 items-start relative">
        <div
          className="hidden xl:flex justify-start items-start w-[200px] max-w-[200px] max-h-[100vh] pt-3 sticky top-12"
        ...
        >
          <motion.div
            ...
            ref={innerRef}
            animate={{
              x: isAtTop ? 8 : `calc(-1*(100vw - 1200px - 64px) / 2)`,
              transition: {
                duration: 0.25,
                ease: "easeInOut",
              },
            ...
            <ul className="mb-0 pl-0 list-none">
              {sections.map((section) => {
                ...
                return (
                  <li
                    key={section.id}
                    className="flex items-start mt-3 font-[14px] leading-[18px] w-full"
                  >
                    <motion.a
                      onClick={() => navigateToSection(section.id)}
                      className="relative flex items-center will-change-transform origin-left my-a cursor-pointer w-full"
                      style={
                        {
                    ...
                    >
                      ...
                      <motion.div
                        ...
                        className="origin-left will-change-transform w-full text-accent-foreground/45 hover:text-accent-foreground"
                      >
                        <span
                          className={`inline transition-colors text-sm duration-250 w-full  ${
        ...
        <div className="flex flex-col gap-10 col-start-2 col-span-1 max-xl:mx-6">
          {children}
        </div>
